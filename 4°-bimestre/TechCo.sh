...
remover_tarefa() {
  if [ -s "$diretorio_tarefas/tarefas.txt" ]; then
    ...
    descricao_tarefa=$(dialog --menu "Escolha uma tarefa para remover:" 12 40 5 "${opcoes[@]}" 3>&1 1>&2 2>&3)

    if [ "$descricao_tarefa" ]; then
      sed -i "/$descricao_tarefa/d;/^ *$/d" "$diretorio_tarefas/tarefas.txt"
      dialog --infobox "Tarefa '$descricao_tarefa' removida com sucesso." 8 40
      sleep 2
    fi
  else
